...
// === Filter prev/next navigation to same category only ===
function filter_portfolio_navigation_same_category($where, $in_same_term, $excluded_terms, $taxonomy, $post) {
    if ($post->post_type === 'portfolio') { // Adjust post type name
        global $wpdb;
        
        // Get current post's primary category
        $categories = get_the_terms($post->ID, 'portfolio_category'); // Adjust taxonomy name
        if (!empty($categories) && !is_wp_error($categories)) {
            $primary_category = $categories[0]; // Use first category as primary
            
            // Modify query to only include posts in same category
            $where .= $wpdb->prepare(" AND p.ID IN (
                SELECT object_id 
                FROM {$wpdb->term_relationships} tr
                INNER JOIN {$wpdb->term_taxonomy} tt ON tr.term_taxonomy_id = tt.term_taxonomy_id
                WHERE tt.term_id = %d AND tt.taxonomy = %s
            )", $primary_category->term_id, 'portfolio_category');
        }
    }
...
// Apply to both previous and next post queries
add_filter('get_previous_post_where', function($where, $in_same_term, $excluded_terms, $taxonomy, $post) {
    return filter_portfolio_navigation_same_category($where, $in_same_term, $excluded_terms, $taxonomy, $post);
}, 10, 5);
